...
#-----------------------------------------------------------------
d_height1 = np.array([0.05]) # np.array([0.1*i/200 for i in range(200)])
...
for i, x in enumerate(d_height):
    m_height[i] = predict(np.matmul(np.array([[0.05, 0.0086, E1, E257, E250, b249, Lv, v]]), max_cols[:-1, :-1]), sparse, T, Cs)
