...
for boundary in boundaries:
    ...
    for j in range(92):
        ...
        Rdotmean = sum(boundary_i_j_k[:, 1])/len(boundary_i_j_k[:, 0])
        Qradmean = sum(boundary_i_j_k[:, 2])/len(boundary_i_j_k[:, 0])
        QoIs.append(np.array([Rdotmean, Qradmean, boundary_i[j, -2], boundary_i[j, -1]]))
